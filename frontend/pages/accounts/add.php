<?php
    ...
    $stmt = $pdo->prepare("SELECT id , full_name FROM clients WHERE utilisateur_id = ?");
    $stmt->execute([$_SESSION['utilisateur']['id']]);
    $clients = $stmt->fetchAll();

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $accountType = $_POST['upload_Account_type--input'];
        $accountAmount = $_POST['upload_compte--amount--input'];
        $accountStatue = $_POST['upload_account_statue--input'];
        $accountOwner = $_POST['upload_account_Owner--input'];

        try {
            $asdb = $pdo->prepare("INSERT INTO comptes (account_number , account_type , solde , account_statue , utilisateur_id , client_id) VALUES (:accountNumber , :accountType , :solde , :accountStatue , :utilisateurId , :clientId)");
            $asdb->execute([
                'accountNumber' => random_int(10000000000000 , 99999999999999),
                'accountType' => $accountType,
                'solde' => $accountAmount,
                'accountStatue' => $accountStatue,
                'utilisateurId' => $_SESSION['utilisateur']['id'],
                'clientId' => $accountOwner
            ]);
                ...
                <div class="w-full flex flex-col gap-10">
                    ...
                    <div class="w-full h-[50%] flex flex-col gap-2.5">
                        ...
                        <div class="w-full border-2 border-[#c0c0c0] rounded-2xl p-[2%] flex items-center">
                            ...
                            <Select class="w-full pl-[2%] outline-0" name="upload_Account_type--input">
                                <option value="" selected disabled>Select type du compte</option>
                    ...
                    <div class="w-full flex flex-col gap-2.5">
                        ...
                        <div class="w-full border-2 border-[#c0c0c0] rounded-2xl p-[2%] flex items-center">
                            ...
                            <input type="number" class="w-full pl-[2%] outline-0" placeholder="Entrer montant initial que le client vous donnez (ex : 1000 MAD)..." required name="upload_compte--amount--input" min="0">
                            <p>MAD</p>
                        </div>
                    </div>
                    <div class="w-full flex flex-col gap-2.5">
                        ...
                        <div class="w-full border-2 border-[#c0c0c0] rounded-2xl p-[2%] flex items-center">
                            ...
                            <select name="upload_account_statue--input" class="w-full pl-[2%] outline-0">
                                ...
                                <option value="Inactif" disabled>Inactif</option>
